...
function parseAmount(amountStr: string): { numeric: number; currency: string } {
  ...
  if (amountStr.includes("€") || amountStr.toLowerCase().includes("eur")) {
    ...
    amountStr.includes("£") ||
    amountStr.toLowerCase().includes("gbp")
  ) {
    currency = "GBP";
...
function transformGrantsForSupabase(
  ...
  return grants.map((grant) => {
    ...
    const id = `${grant.organization
      ?.toLowerCase()
      .replace(/\s+/g, "-")}-${grant.title
      ?.toLowerCase()
...
export async function syncGrantsToSupabase(): Promise<void> {
  try {
    ...
    console.log(
      `[GrantsSync] Received ${sonarGrants.length} grants from Sonar`
    );
    ...
    if (grantsForSupabase.length > 5) {
      console.log(
        `[GrantsSync] Limiting from ${grantsForSupabase.length} to 5 grants`
      );
      grantsForSupabase = grantsForSupabase.slice(0, 5);
    }

    console.log(
      `[GrantsSync] Transformed ${grantsForSupabase.length} grants for Supabase`
    );
    ...
    console.log(
      `[GrantsSync] Successfully inserted ${grantsForSupabase.length} grants to Supabase`
    );

    // Try to refresh materialized view if it exists
    console.log(
      "[GrantsSync] Attempting to refresh popular_open materialized view..."
    );
    const { error: refreshError } = await supabaseAdmin.rpc(
      "refresh_popular_open"
    );

    if (refreshError) {
      console.warn(
        "[GrantsSync] Materialized view refresh warning:",
